...
durations at nanosecond precision before SDK defaults/conversion, requiring explicit free policy
and total/used/reserved values. Floor total, ceil used/reserved and clamp remaining to zero;
mark lower-bound seconds. Missing data stays unknown, errors unavailable, reset absent. Timestamp
before work and preserve provenance when ageing; stale/unknown data cannot clear exhaustion.
...
Defaults, upward rounding, omitted reservations, guessed reset times and latest log/session targets
invent operational facts. Conservative observations can withhold compatibility but preserve account
and attempt safety. Quota is not reservation, logs are not artifacts, and merged code is not live
verification. See [operational mappings](../providers/kaggle-operations.md).
