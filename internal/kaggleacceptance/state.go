...
func (s *session) close() error {
	...
	if s.results != nil { err = errors.Join(err, s.results.Close()) }
	if s.inputs != nil { err = errors.Join(err, s.inputs.Close()) }
	if s.store != nil { err = errors.Join(err, s.store.Close()) }
	if s.root != nil { err = errors.Join(err, s.root.Close()) }
	return err
}
...
func openSession(ctx context.Context, o Options, fixture bool) (result *session, err error) {
	...
	if create {
		if _, err = statefs.PrivateDir(o.Root, true); err != nil { return nil, ErrState }
	} else if statefs.CheckDir(o.Root) != nil {
		return nil, ErrState
	}
	s.root, err = statefs.Open(o.Root)
	if err != nil { return nil, ErrState }
	root := s.root.Path
	if !create {
		if readRecord(filepath.Join(root,"acceptance.json"), &s.record) != nil { return nil, ErrState }
		for _, name := range []string{"state","inputs","results"} {
			if statefs.CheckDir(filepath.Join(root,name)) != nil { return nil, ErrState }
		}
	...
	if err != nil { return nil, ErrState }
	if create { s.record.InputSHA256 = provider.Digest(input) }
	...
	if err != nil { return nil, err }
	if create {
		if err := s.store.PutWorkspace(ctx, auth.Workspace{ID:Workspace, Enabled:true, AllowedProfiles:[]string{ProfileName}}); err != nil { return nil, err }
		...
		if err := s.store.PutProfile(ctx, profile, true); err != nil { return nil, err }
		if err := s.store.ConfigureScheduler(ctx, scheduler.DefaultSettings()); err != nil { return nil, err }
		for id, data := range map[domain.ObjectID][]byte{"acceptance_code":bundle,"acceptance_input":input} {
			...
			if err != nil { return nil, err }
			if err := s.store.CommitObject(ctx, metadata); err != nil { return nil, err }
		}
	}
	...
	if create {
		...
		if err != nil { return nil, err }
		if err := writeRecord(filepath.Join(root,"acceptance.json"), s.record); err != nil { return nil, err }
	}
	...
	for _, object := range original.Objects {
		...
		if object.Role != "bundle" { expected = s.record.InputSHA256 }
		if object.Object.SHA256 != expected { return nil, ErrState }
	}
	return s, nil
}

func (s *session) journal(ctx context.Context) (dispatch.Journal, error) {
	return s.store.DispatchJournal(ctx, Workspace, s.record.Receipt.JobID, s.record.Receipt.AttemptID)
}
func (s *session) original(ctx context.Context) (provider.Plan, provider.Prepared, error) {
	j, err := s.journal(ctx)
	if err != nil || j.Plan == nil || j.Prepared == nil { return provider.Plan{}, provider.Prepared{}, ErrState }
...
func (r *markedRepository) CommitDispatch(ctx context.Context, handle dispatch.Handle, action dispatch.Action, now time.Time) (dispatch.Work, error) {
	...
	if action.Kind == dispatch.BeginSubmission {
		if work.Journal.Plan == nil || !work.Journal.SubmitStarted { return dispatch.Work{}, ErrState }
		mark := submissionMark{1,r.process,work.Journal.Plan.Digest(),work.Journal.Plan.Job.Identity.IntentID,now.UTC()}
		if err := writeRecord(filepath.Join(r.s.root.Path,"submission-process.json"), mark); err != nil { return dispatch.Work{}, err }
	}
	return work, nil
}
